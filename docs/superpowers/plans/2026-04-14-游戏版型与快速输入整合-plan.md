...
**Architecture:** SetupActivity选择人数和版型，MainActivity接收人数动态渲染玩家，修改dialog_add_speech.xml添加模板按钮，新增二级弹窗布局，处理模板点击逻辑。

**Tech Stack:** Kotlin, Android SDK 34, MVVM, ViewBinding, Material Design

---

## Implementation Notes (实际实施变更)

以下内容在实际实施中已调整，与原计划有所不同：

### UX改进：人数选择移至SetupActivity

**原因：** 简化MainActivity界面，避免加减按钮干扰游戏操作

**变更内容：**
1. `activity_setup.xml` - 新增人数选择按钮区域
   - 4个按钮使用layout_weight均分屏幕宽度
   - 选中效果：亮青色背景(teal_200) + 黑色文字
   - 未选中效果：深色背景 + 灰色文字 + 边框

2. `SetupActivity.kt` - 添加人数选择逻辑
   - `selectedPlayerCount` 变量存储选择
   - `selectPlayerCount()` 方法处理选中状态切换
   - Intent传递PLAYER_COUNT给MainActivity

3. `GameViewModel.kt` - 新增`initializePlayers(count)`方法

4. `MainActivity.kt` - 接收人数Intent，移除加减按钮逻辑

5. `activity_main.xml` - 移除btnAddPlayer

**相关提交：**
- `350aba5` - feat: 将人数选择移至SetupActivity，简化MainActivity界面
- `e939f19` - style: SetupActivity人数选择按钮均分屏幕宽度
- `1da1e42` - style: 增强人数选择按钮选中效果对比度
...
- `app/src/main/res/layout/dialog_mark_role.xml` - 铅笔图标角色选择弹窗

**修改文件：**
- `app/src/main/res/layout/activity_setup.xml` - 新增人数选择按钮区域（均分宽度）
- `app/src/main/java/com/voicewolf/app/SetupActivity.kt` - 人数选择逻辑，Intent传递
- `app/src/main/java/com/voicewolf/app/GameViewModel.kt` - initializePlayers(count)方法
- `app/src/main/java/com/voicewolf/app/Player.kt` - getMarkedRoleDisplayName扩展函数
- `app/src/main/res/layout/activity_main.xml` - 右列改为动态容器，移除静态player7-12，移除btnAddPlayer
- `app/src/main/res/layout/dialog_add_speech.xml` - 新增模板按钮网格布局
- `app/src/main/java/com/voicewolf/app/MainActivity.kt` - 接收人数Intent，动态玩家渲染，模板按钮处理，铅笔图标点击，二级弹窗逻辑
